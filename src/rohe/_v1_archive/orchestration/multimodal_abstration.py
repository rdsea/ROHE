...
class MonitoringReport(BaseModel):
    metadata: dict | None = Field(
        None, description="Metadata for the monitoring report"
    )
    query_id: str = Field(..., description="Unique identifier for the query")
    inf_id: str | None = Field(None, description="Inference ID for the report")
    inf_time: float = Field(..., description="Inference time in seconds")
    data_source: list[str] | str | None = Field(
        ..., description="Data sources used in the inference"
    )
    ...
    model_version: str | None = Field(
        None, description="Version of the model used for the inference"
    )
    explainability: bool | None = Field(
        False, description="Whether the report includes explainability data"
    )
    time_violation: bool | None = Field(
        False, description="Whether the report includes time violation data"
    )
    time_for_inference: float | None = Field(
        None, description="Remaining time for inference in seconds"
    )
...
class Metric(BaseModel):
    ...
    condition: str | None = Field(
        None, description="Condition for the metric (e.g., confidence threshold)"
    )
        ...
        if "overall_performance" in data and isinstance(
            ...
            try:
                data["overall_performance"] = [
                    Metric(**item) for item in data["overall_performance"]
                ]
            except Exception as e:
        ...
        if "class_specific_performance" in data and isinstance(
            ...
            try:
                data["class_specific_performance"] = [
                    ClassSpecificMetric.from_dict(item)
...
class InferenceServiceProfile(BaseModel):
    metadata: dict | None = Field(
        None, description="Metadata for the inference service profile"
    )
    inference_service_id: str = Field(
        ..., description="Unique identifier for the inference service"
    )
    model_id: str = Field(..., description="Model ID used in the inference service")
    model_version: str | None = Field(
        None, description="Version of the model used in the inference service"
    )
    device_type: str = Field(
        ..., description="Type of device used for the inference service"
    )
    base_line: list[Metric] = Field(
        ..., description="Baseline performance metrics for the inference service"
    )
    inference_performance: RuntimePerformance | None = Field(
        None, description="Runtime performance metrics for the inference service"
    )
    instance_list: list[str] = Field(
        ..., description="List of instance_id for the inference service"
    )
    specialized_class: list[str] | None = Field(
        None, description="List of specialized classes for the inference service"
    )
    modality: str | None = Field(
        None, description="Modality of the inference service (e.g., 'image', 'text')"
    )
...
class InferenceServiceInstance(BaseModel):
    instance_id: str = Field(
        ..., description="Unique identifier for the inference service instance"
    )
    model_id: str | None = Field(
        ..., description="Model ID used in the inference service instance"
    )
    model_version: str | None = Field(
        None, description="Version of the model used in the inference service instance"
    )
    device_id: str = Field(
        ..., description="Device ID where the inference service instance is running"
    )
    ip_address: str = Field(
        ...,
        description="IP address of the device running the inference service instance",
    )
    port: int = Field(..., description="Port number for the inference service instance")
    runtime_performance: list[Metric] = Field(
        ...,
        description="Runtime performance metrics for the inference service instance",
    )
    modality: str | None = Field(
        None,
        description="Modality of the inference service instance (e.g., 'image', 'text')",
    )
    device_type: str | None = Field(
        None, description="Type of device used for the inference service instance"
    )
    inference_service_id: str | None = Field(
        None, description="Inference service ID associated with the instance"
    )
    status: str | None = Field(
        None,
        description="Status of the inference service instance (e.g., 'active', 'inactive', 'error', 'contention')",
    )
    inference_url: str | None = Field(
        None, description="URL for the inference service instance"
    )
        ...
        if "runtime_performance" in data and isinstance(
            ...
            try:
                data["runtime_performance"] = [
                    Metric(**item) for item in data["runtime_performance"]
                ]
            except Exception as e:
...
class ServiceLevelIndicator(BaseModel):
    ...
    target_value: float | list[float] | None = Field(
        None, description="Target value(s) for the service level indicator"
    )
    operator: str | None = Field(
        None,
        description="Operator for the service level indicator (e.g., '>', '<', '==')",
    )
    objective_type: str | None = Field(
        None, description="Type of the objective (e.g., 'minimize', 'maximize')"
    )
    condition: str | None = Field(
        None, description="Condition for the objective (e.g., 'confidence threshold')"
    )
    class_id: str | None = Field(
        None, description="Class id for the objective (if applicable)"
    )
...
class ServiceLevelAgreement(BaseModel):
    ...
    access_privileges: list[str] = Field(
        ..., description="List of data sources the tenant has access to"
    )
    service_level_indicators: list[ServiceLevelIndicator] = Field(
        ..., description="List of service level indicators for the SLA"
    )
    consumer_list: list[str] | None = Field(
        None, description="List of consumers associated with the SLA"
    )
    ensemble_size: int | None = Field(1, description="Ensemble size for the inferences")
    ensemble_selection_strategy: str | None = Field(
        "enhance_confidence", description="Ensemble selection strategy for the tenant"
    )
...
class MonitoringClient(BaseModel):
    ...
    def send_report(self, report: MonitoringReport):
        try:
            ...
            connection.close()
            logging.info(
                f"Monitoring report sent successfully for query_id: {report.query_id}"
            )
            return True
        except Exception as e:
...
class InferenceTask(BaseModel):
    ...
    modality: str | None = Field(
        None, description="Modality of the task (e.g., 'image', 'text')"
    )
    allocated_time: float | None = Field(
        None, description="Allocated time for the task in seconds"
    )
    min_execution_time: float | None = Field(
        None, description="Minimum execution time for the task in seconds"
    )
    max_execution_time: float | None = Field(
        None, description="Maximum execution time for the task in seconds"
    )
    ...
    inference_services: dict | None = Field(
        {}, description="Inference services associated with the task"
    )
    inference_instances: dict | None = Field(
        {}, description="Inference instances associated with the task"
    )
    inference_services_ex: dict | None = Field(
        {}, description="Inference services associated with the task for explainability"
    )
    inference_instances_ex: dict | None = Field(
        {},
        description="Inference instances associated with the task for explainability",
    )
    status: str | None = Field(
        None,
        description="Status of the task (e.g., 'pending', 'running', 'completed', 'error')",
    )
    phase: int | None = Field(0, description="Phase of the task")
    inference_query: InferenceQuery | None = Field(
        None, description="Inference query associated with the task"
    )
    selected_instances: list[InferenceServiceInstance] | None = Field(
        [], description="List of selected inference service instances for the task"
    )
    selected_instances_ex: list[InferenceServiceInstance] | None = Field(
        [],
        description="List of selected inference service instances for the task in explainability mode",
    )
    ensemble_size: int | None = Field(1, description="Ensemble size for the task")
    ensemble_selection_strategy: str | None = Field(
        "enhance_confidence", description="Ensemble selection strategy for the task"
    )
    test_mode: bool | None = Field(True, description="Whether the task is in test mode")
    monitoring_client: MonitoringClient | None = Field(
        None, description="Monitoring client for the task"
    )
    explainability: bool | None = Field(
        False, description="Whether to include explainability in the task"
    )
            ...
            with ThreadPoolExecutor(max_workers=self.ensemble_size) as executor:
                ...
                if self.explainability:
                    for instance_ex_i in self.selected_instances_ex:
                        inference_url = (
                            instance_ex_i.inference_url
                            if instance_ex_i.inference_url
                            else DEFAULT_INFERENCE_URL
                        )
                ...
                for instance_i in self.selected_instances:
                    inference_url = (
                        instance_i.inference_url
                        if instance_i.inference_url
                        else DEFAULT_INFERENCE_URL
                    )
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            response_explainability = dict_response.get(
                                "explainability", False
                            )
                            logging.debug(
                                f"Executed inference for task {self.task_id} with {inference_result}"
                            )
                            if isinstance(self.monitoring_client, MonitoringClient):
                                ...
                                monitoring_report_dict = {
                                    ...
                                    "response_time": dict_response.get(
                                        "response_time", 0.0
                                    ),
                                ...
                                monitoring_report = MonitoringReport.model_validate(
                                    monitoring_report_dict
                                )
                                self.monitoring_client.send_report(monitoring_report)
                        else:
                            logging.error(
                                f"Failed to execute inference for task {self.task_id} on instance {instance_i.instance_id}: {response.status_code} - {response.text}"
                            )
                    except Exception as e:
                        logging.error(
                            f"Error occurred while executing inference for task {self.task_id} on instance {instance_i.instance_id}: {e}"
                        )
                        logging.error(traceback.format_exc())

            logging.debug(
                f"Aggregated inference result for task {self.task_id}: {aggregated_inference_results}"
            )
...
class TaskList(BaseModel):
    data: list[InferenceTask] | None = Field([], description="List of task information")
    ...
    def add_task(self, inf_task: InferenceTask):
        ...
        if self.get_task_info(inf_task.task_id) is not None:
            raise ValueError(
                f"Task with ID {inf_task.task_id} already exists in the task list"
            )
...
class InferenceResult(BaseModel):
    ...
    query_id: list[str] | str | None = Field(
        None, description="List of query IDs associated with the inference result"
    )
    task_id: list[str] | None = Field(
        [], description="List of task IDs associated with the inference result"
    )
    inf_id: list[str] | None = Field(
        [], description="List of inference IDs associated with the inference result"
    )
    metadata: dict | None = Field(
        {}, description="Metadata associated with the inference result"
    )
    ...
    def get_top_k_predictions(self, top_k: int = 5) -> list[str]:
        ...
        top_k = sorted(self.data.items(), key=lambda item: item[1], reverse=True)[
            :top_k
        ]
    ...
    def aggregate(
        ...
        if self.query_id is None and other.query_id is None:
            raise ValueError(
                "query_id of both InferenceResult instances must be set for aggregation"
            )
        if self.query_id is None:
            self.query_id = other.query_id
        if self.query_id != other.query_id:
            raise ValueError(
                "query_id of both InferenceResult instances must match for aggregation"
            )
        ...
        for class_name in new_class_list:
            if avg_flag:
                ...
                aggregate_data[class_name] = (
                    self.data.get(class_name, 0) + other.data.get(class_name, 0)
                ) / 2
            else:
                # sum the values for each class
                aggregate_data[class_name] = self.data.get(
                    class_name, 0
                ) + other.data.get(class_name, 0)
