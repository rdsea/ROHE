...
class ProcessingServiceExecutor(RoheObject):
    def __init__(self, config: dict, log_level=2):
        ...
        self.minio_connector = MinioConnector(
        ...
        self.image_processing_func: Callable = pipeline_utils.get_function_from_module(
            module=image_processing_func,
    ...
    def change_image_processing_function(self, func_name: str) -> bool:
        ...
        func: Callable = pipeline_utils.get_function_from_module(
            module=image_processing_func, func_name=func_name
        )
        if func is not None:
    ...
    def _processing(self, task: dict):
        ...
        if response.status_code != 200:
            ...
            logging.info(
                f"Make request to inference servers: {self.inference_server_urls}"
            )
    ...
    async def _make_requests(self, payload, files):
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [
                self._post_request(session, server, payload, files)
                for server in self.inference_server_urls
            ]
    ...
    async def _post_request(self, session, url, payload, files):
        try:
            ...
            for file_name, file_tuple in files.items():
                ...
                data.add_field(
                    file_name, file_bytes, filename=filename, content_type=content_type
                )

            print(
                f"send request to this server {url} at timestamp {pipeline_utils.get_current_utc_timestamp()}"
            )
            async with session.post(url, data=data) as response:
                ...
                if response.status == 200:
                    ...
                    message = await response.json()
                    logging.error(
                        f"Inference failed with status code: {response.status}, message: {message}"
                    )
