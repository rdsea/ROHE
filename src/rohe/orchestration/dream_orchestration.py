    ...
    def determine_execution_workflow(self, inference_query: InferenceQuery, task_list: TaskList, start_time: float, e2e_response_time: float = 5):
        try:
            ...
            for task in task_list.data:
                logging.debug(f"DREAM - Modality: {task.modality}, Services: {len(task.inference_services)}, Instances: {len(task.inference_instances)}, Min Execution Time: {task.min_execution_time}, Max Execution Time: {task.max_execution_time}, Allocated Time: {task.allocated_time}, Phase: {task.phase}, Status: {task.status}")
            return task_list
