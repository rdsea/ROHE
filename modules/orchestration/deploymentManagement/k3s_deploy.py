import os, yaml, json, copy, argparse
...
for app_key in deploy_conf:
    ...
    for ser_key in application:
        ...
        for node in service['node']:
            ...
            temp_file['metadata']['name'] = temp_file['metadata']['name'].replace('node_name', node['node_name'])
            
            if node['node_name'] == "all":
                temp_file['spec']['template']['spec'].pop('nodeSelector', None)
            else:
                temp_file['spec']['template']['spec']['nodeSelector']['kubernetes.io/hostname'] = temp_file['spec']['template']['spec']['nodeSelector']['kubernetes.io/hostname'].replace('node_name', node['node_name'])
            temp_file['spec']['replicas'] = node['replicas']
            with open('./deployed/'+app_key+'-'+ser_key+'-'+node['node_name']+'.yml', 'w+') as outfile:
