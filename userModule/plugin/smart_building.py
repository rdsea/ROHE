...
def accurate_explainability(row, k: int) -> int:
    try:
        ...
        top_k_predictions = dict(
            sorted(inference_data.items(), key=lambda x: x[1], reverse=True)[:k]
        )
        # Calculate accuracy
        ...
        logging.warning(f"Error evaluating explainability accuracy: {e}")
...
def explainability_sli_check(
    monitoring_data: str | pd.DataFrame,
    ground_truth: str | pd.DataFrame,
    top_k: int = 1,
    window_size: int = 5,
    step_size: int = 1,
) -> float:
    if isinstance(monitoring_data, pd.DataFrame) and isinstance(
        ...
        explainability_df["top_k_explainability"] = explainability_df.apply(
            lambda row: accurate_explainability(row, top_k), axis=1
        )
