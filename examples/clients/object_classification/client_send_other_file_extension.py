...
from examples.applications.NII.utilities.mqttPublisher import MqttPublisher
...
def main(config):
    publisher = MqttPublisher(
        broker_info=config["mqtt_config"],
        client_id=config["device_id"],
        pub_topic=config["mqtt_config"]["topic"],
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--conf", type=str, help="configuration file", default="iot_device.json"
    )
    parser.add_argument(
        "--test_ds",
        type=str,
        help="default test dataset path",
        default="test_image/02.jpg",
    )
    ...
    parser.add_argument(
        "--rate", type=int, help="default number of requests per second", default=20
    )
