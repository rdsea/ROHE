...
class RoheRegistration(RoheRestObject):
    ...
    def generate_agent_conf(self, metadata) -> dict:
        try:
            ...
            agent_db_config.database = "application_"+metadata["application_name"]+"_"+metadata["appID"]
            ...
            if request.is_json:
                ...
                if "application_name" in args:
                    ...
                    if app == None:
                        ...
                        response[application_name] = "Application {} created for user {} with run ID: {}".format(application_name, user_id, run_id)
                    ...
                    if "user_id" in args:
                        i_config["out_routing_key"] = i_config["out_routing_key"]+"."+args["user_id"]
                    if "stage_id" in args:
                        i_config["out_routing_key"] = i_config["out_routing_key"]+"."+args["stage_id"]
                    if "instance_id" in args:
                        i_config["out_routing_key"] = i_config["out_routing_key"]+"."+args["instance_id"]
