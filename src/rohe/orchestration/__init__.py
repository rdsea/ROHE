...
def create_orchestrator(algorithm: str = "adaptive", config_path: str = "../config/orchestrator.yaml"):
    class_path = ORCHESTRATOR_REGISTRY.get(algorithm.lower())
    if class_path is None:
        raise ValueError(f"Unknown orchestration algorithm '{algorithm}'. Available: {list(ORCHESTRATOR_REGISTRY.keys())}")
