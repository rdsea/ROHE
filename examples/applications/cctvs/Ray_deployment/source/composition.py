...
def enhance_image(image):
    ...
    enh_img_qoa.init_report(
        enh_img_conf["instance_id"],
        enh_img_conf["functionality"],
        enh_img_conf["stage_id"],
    )
    enh_img_qoa.timer()
...
def max_aggregate(predictions, report_list=[]):
    ...
    m_agg_qoa.init_report(
        m_agg_conf["instance_id"], m_agg_conf["functionality"], m_agg_conf["stage_id"]
    )
    m_agg_qoa.timer()
...
class Yolo8Inference:
    def __init__(self, param):
        ...
        self.qoa.init_report(
            self.conf["instance_id"], self.conf["functionality"], self.conf["stage_id"]
        )
...
if last_updated_u_data != None:
    ...
    for model in ensemble_model:
        i_query = {"id": model}
        instance_data = instance_collection.find(i_query).sort(
            [("timestamp", pymongo.DESCENDING)]
        )
        try:
            ...
            conf.append(
                {
                    "model": last_updated_i_data["model"],
                    "parameter": last_updated_i_data["parameter"],
                }
            )
