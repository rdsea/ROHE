...
def preprocess_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "confidence" in processed_df.columns:
        ...
        processed_df = processed_df[
            processed_df["confidence"].between(0, 1, inclusive=True)
        ]

    # Convert to appropriate data types
    processed_df["timestamp"] = pd.to_numeric(
...
def get_throughtput_info(
    dataframe: pd.DataFrame, window_size: int = 100, step_size: int = 1
) -> dict:
...
def get_label_distribution(dataframe: pd.DataFrame, class_label_dict: dict) -> dict:
    ...
    label_dist = {"total_sample": len(dataframe), "label_dist": label_dist}
    return label_dist


def get_inference_quality_info(
    dataframe: pd.DataFrame, class_label_dict: dict
) -> pd.DataFrame:
    dataframe = dataframe.copy(deep=True)
    ...
    avg_confidence_correct = correct_predictions.groupby("class_object")[
        "confidence"
    ].mean()

    # For avg_confidence_incorrect_prediction
    incorrect_predictions = dataframe[dataframe["accuracy"] == 0]
    avg_confidence_incorrect = incorrect_predictions.groupby("class_object")[
        "confidence"
    ].mean()

    # For percentage_confidence_over_50
    conf_over_50 = (
        dataframe[dataframe["confidence"] > 0.5].groupby("class_object").size()
    )
    total_counts = dataframe.groupby("class_object").size()
    percent_conf_over_50 = conf_over_50 / total_counts

    print(
        f"\n\n\nThis is percent conf over 50: {percent_conf_over_50}, the type is: {type(percent_conf_over_50)}"
    )  # Create a new DataFrame to hold the calculated metrics
    print(
        f"\n\n\nThis is confidence metric: {grouped['confidence'].min()}, the type is: {type(grouped['confidence'].min())}"
    )  # Create a new DataFrame to hold the calculated metrics
