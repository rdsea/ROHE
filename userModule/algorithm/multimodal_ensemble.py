...
def get_function_from_plugin(app_name: str, plugin_name: str):
    try:
        ...
        logging.error(
            f"Error getting function {plugin_name} from plugin {app_name}: {e}"
        )
        return None
...
def enhance_confidence(
    ...
    try:
        ...
        used_services = []
        top_k_function = get_function_from_plugin(
            app_name=app_name, plugin_name=plugin_name
        )
        top_k_classes = top_k_function(intermediate_result.data, top_k=top_k)
        if top_k > len(top_k_classes):
            logging.debug(
                f"Requested top_k {top_k} is greater than available classes {len(top_k_classes)} selecting k classes by default."
            )
            ...
            if i >= len(top_k_classes):
                for key, value in inference_services.items():
                    ...
                    for metric in overall_performance:
                        ...
                        if (
                            metric_name == CommonMetric.ACCURACY.value
                            and metric_value > best_accuracy
                            and key not in used_services
                        ):
                ...
                if i == 0 and task.explainability:
                    for key, value in task.inference_services_ex.items():
                        ...
                        for metric in overall_performance:
                            ...
                            if (
                                metric_name == CommonMetric.ACCURACY.value
                                and metric_value > best_accuracy_ex
                                and key not in used_services
                            ):
                ...
                for key, value in inference_services.items():
                    ...
                    class_specific_performance = (
                        inference_performance.class_specific_performance
                    )
                            ...
                            if class_name == top_i_class:
                                ...
                                for metric in i_performance:
                                    ...
                                    if (
                                        ...
                                        selected_service_ex_i = key
            logging.debug(
                f"Selected service {selected_service_i} with accuracy {best_accuracy} for task {task.task_id} at iteration {i}."
            )
            if selected_service_i:
                used_services.append(selected_service_i)
                instance_list_i = inference_services.get(
                    selected_service_i
                ).instance_list
                ...
                for instance_key_j in instance_list_i:
                    if instance_key_j in list(inference_instances.keys()):
                        ...
                        for metric_k in runtime_performance_j:
                            if (
                                metric_k.metric_name == CommonMetric.RESPONSE_TIME.value
                                and metric_k.condition == Explainability.DISABLED.value
                            ):
                ...
                if selected_instance_i:
                    ...
                    logging.debug(
                        f"Selected instance {selected_instance_i.instance_id} for service {selected_service_i} in task {task.task_id}."
                    )
                else:
                    logging.warning(
                        f"No suitable instance found for service {selected_service_i} in task {task.task_id}."
                    )
            else:
                logging.warning(
                    f"No suitable service found for task {task.task_id} at iteration {i}."
                )
            if i == 0 and task.explainability and selected_service_ex_i:
                used_services.append(selected_service_ex_i)
                instance_list_ex_i = task.inference_services_ex.get(
                    selected_service_ex_i
                ).instance_list
                ...
                for instance_key_j in instance_list_ex_i:
                    if instance_key_j in list(inference_instances.keys()):
                        ...
                        for metric_k in runtime_performance_j:
                            if (
                                metric_k.metric_name == CommonMetric.RESPONSE_TIME.value
                                and metric_k.condition == Explainability.ENABLED.value
                            ):
                                response_time_k = metric_k.value
                ...
                if selected_instance_ex_i:
                    ...
                    logging.debug(
                        f"Selected explainable instance {selected_instance_ex_i.instance_id} for service {selected_service_ex_i} in task {task.task_id}."
                    )
                else:
                    logging.warning(
                        f"No suitable explainable instance found for service {selected_service_ex_i} in task {task.task_id}."
                    )

        logging.debug(
            f"Task {task.task_id} selected {len(task.selected_instances)} instances out of requested top_k {top_k}."
        )
        if len(task.selected_instances) < top_k:
            logging.warning(
                f"Not enough instances selected for task {task.task_id}. Selected {len(task.selected_instances)} out of {top_k}."
            )
    except Exception as e:
...
def select_by_overall_accuracy(
    task: InferenceTask, intermediate_result: InferenceResult, top_k: int = 1
):
    try:
        task.selected_instances = []
        used_services = []
        ...
        logging.error(
            f"Error in select_by_overall_accuracy for task {task.task_id}: {e}"
        )
        logging.error(traceback.format_exc())


def enhance_generalization(
    task: InferenceTask, intermediate_result: InferenceResult, worst_k: int = 1
):
    try:
        task.selected_instances = []
        used_services = []
        ...
        if worst_k > len(worst_k_classes):
            logging.debug(
                f"Requested worst_k {worst_k} is greater than available classes {len(worst_k_classes)} selecting k classes by default."
            )
        inference_instances = task.inference_instances
        inference_services = task.inference_services
        ...
        logging.debug(
            f"Task {task.task_id} selected {len(task.selected_instances)} instances out of requested worst_k {worst_k}."
        )
        if len(task.selected_instances) < worst_k:
            logging.warning(
                f"Not enough instances selected for task {task.task_id}. Selected {len(task.selected_instances)} out of {worst_k}."
            )
    except Exception as e:
        logging.error(f"Error in enhancing confidence for task {task.task_id}: {e}")
        logging.error(traceback.format_exc())
