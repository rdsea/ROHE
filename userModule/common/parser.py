    ...
    try:
        ...
        for item in data:
            ...
            if "quality" in item:
                ...
                confidence = item["quality"]["inference"][last_inf]["confidence"][
                    "value"
                ]
            # Get metadate for later analyses
            if "metadata" in item:
        ...
        if "quality" in item:
            ...
            if ("inference" in metric_list) and ("inference" in quality_report):
                ...
                for inf_metric in metric_list["inference"]:
                    metric_cols.update(
                        get_inference_metric(quality_report["inference"], inf_metric)
                    )
            if ("data" in metric_list) and ("data" in quality_report):
                # Get data quality metrics
                for data_metric in metric_list["data"]:
                    metric_cols.update(
                        get_data_quality_metric(quality_report["data"], data_metric)
                    )
...
def sdnParser(item, parser_conf):
    try:
        ...
        for key in switches:
            ...
            switch = switches[key]
            row_switch.update(
                aggregate_switch(switch["FlowStats"], flowstat_config, "flow_")
            )
            row_switch.update(
                aggregate_switch(switch["PortStats"], portstat_config, "port_")
            )
