...
def generate_possible_deployment(model_list, infrastructure_list):
    ...
    while running_flag:
        ...
        for i in range(len(model_list)):
            deployment.append({"model": model_list[i], "infrastructure": infrastructure_list[deployment_array[i]]})
...
def scale_reduction(model_list: list, infrastructure_list: list, ml_performance_list: list, objective_func_name: str, contract: dict, to_df: bool = False):
    ...
    for ensemble_num in range(min_ensemble_element, max_ensemble_element+1):
        for ensemble in combinations(model_list, ensemble_num):
            possible_deployment = list(generate_possible_deployment(list(ensemble), infrastructure_list))
            for deployment in possible_deployment:
                ...
                for service in deployment:
                    ...
                    for ml_service in ml_performance_list:
                        if (ml_service.model == service["model"] and ml_service.infrastructure == service["infrastructure"]):
