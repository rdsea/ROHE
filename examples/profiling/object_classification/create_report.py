...
if __name__ == "__main__":
    ...
    for subfolder in subfolders:
        ...
        provider_df_path = os.path.join(
            profile_folder_path, subfolder, provider_file_name
        )
    ...
    for model_id, data in dict_object.items():
        ...
        risk_level_info = profiling_utils.get_risk_levels_info(
            dataframe=processed_client, class_label_dict=class_label_dict
        )

        model_info = profiling_utils.get_model_info_from_provider(
            dataframe=processed_provider
        )
        label_dist = profiling_utils.get_label_distribution(dataframe=processed_client)

        inference_quality_info = profiling_utils.get_inference_quality_info(
            dataframe=processed_client, class_label_dict=class_label_dict
        )

        overall_quality_info = profiling_utils.get_overall_inference_quality_info(
            dataframe=processed_client
        )
        ...
        info = {
            **identity,
            **client_throughput_info,
            **model_info,
            **risk_level_info,
            **label_dist,
            **overall_quality_info,
            **resource,
        }
