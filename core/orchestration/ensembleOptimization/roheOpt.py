...
class RoheEnsembleOptimization(EnsembleOptimization):
    ...
    def get_ml_service_performance(self, database , model_collection: str, infrastructure_collection: str, model_cost_collection: str, infrastructure_cost_collection: str, metricConf: dict, timestamp = None, limit = 10000) -> dict:
        ...
        for model_name in self.model_list:
            ...
            for key, metric in metricConf.items():
                metrics[key] = execute_metric_queries(self.model_collection, metric, model_name, limit=limit, timestamp=timestamp)
    ...
    def select(self, mlServiceList: list, contract: dict) -> list:
        ...
        ensemble = self.optimization_algorithm(self.model_list, self.infrastructure_list, mlServiceList, self.objective_funtion, contract)
        return ensemble
