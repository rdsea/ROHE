...
class Observability_Agent(object):
    ...
    def aggreagate_resource(self, df_resource):
        ...
        try:
            if not df_resource.empty:
                output = {"mean_cpu_user":df_resource["cpu_user"].mean(),
                        "mean_cpu_system":df_resource["cpu_system"].mean(),
                        "avg_mem_rss":df_resource["mem_rss"].mean(),
                        "avg_mem_vms":df_resource["mem_vms"].mean()}
                return output
        except Exception as e:
            print("[ERROR] - Error {} while estimating resource report: {}".format(type(e),e.__traceback__))
            traceback.print_exception(*sys.exc_info())
    ...
    def estimate_contribution(self):
        ...
        sumary = {}
        try:
            quality_report = self.quality_temp_col.find().sort([('timestamp', pymongo.DESCENDING)])
            resource = self.resource_temp_col.find().sort([('timestamp', pymongo.DESCENDING)])
            feedback = self.feedback_temp_col.find().sort([('timestamp', pymongo.DESCENDING)])
            ...
            if len(df_contr.index) == 0:
                ...
                with open(output_folder+"sumary_at_"+str(time.time())+".json", "w") as outfile:
