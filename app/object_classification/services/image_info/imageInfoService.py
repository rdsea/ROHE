...
class ImageInfoService(RoheRestObject):
    ...
    def _claim_image(self):
        serialized_image_info = self.redis.rpoplpush(
        ...
        if serialized_image_info:
            ...
            return (
                json.dumps({"image_info": image_info}),
                200,
                {"Content-Type": "application/json"},
            )
        else:
            return (
                json.dumps({"status": "no unprocessed images"}),
                404,
                {"Content-Type": "application/json"},
            )
    ...
    def _add_image(self, request: request):
        ...
        if all(image_info[field] is not None for field in required_fields):
            ...
            return (
                json.dumps(
                    {
                        "error": f"Some required fields are missing, the required field are: {required_fields}"
                    }
                ),
                400,
                {"Content-Type": "application/json"},
    ...
    def _complete_processing(self, request: request):
        ...
        if image_info:
            ...
            return (
                json.dumps(
                    {
                        "error": "Image info is not valid. Or the image is already processed and report by another instance. Do not need to repeat"
                    }
                ),
                400,
                {"Content-Type": "application/json"},
            ...
            return (
                json.dumps({"error": "Image info is missing"}),
                400,
                {"Content-Type": "application/json"},
            )
