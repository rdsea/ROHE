...
class Service(object):
    ...
    def __init__(self, kube_config, service_config, namespace):
        self.client = dynamic.DynamicClient(api_client.ApiClient(configuration=kube_config))
    ...
    def create(self):
        try:
            self.service = self.api_client.create(body=self.service_config, namespace=self.namespace)
    ...
    def get(self):
        try:
            self.service = self.api_client.create(name=self.name, namespace=self.namespace)
            print("Service created: %s" % self.service)
            return self.service
        except Exception as e:
            print("Exception when calling service create: %s\n" % e)
    ...
    def update(self, service_config):
        try:
            self.service = self.api_client.patch(body=service_config, name=self.name, namespace=self.namespace)
            print("Service created: %s" % self.service)
        except Exception as e:
    ...
    def delete(self):
        try:
            api_response = self.api_client.delete(body={}, name=self.name, namespace=self.namespace)
