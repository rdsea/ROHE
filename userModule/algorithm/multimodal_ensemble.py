...
def select_by_overall_accuracy(task: InferenceTask, intermediate_result: InferenceResult, top_k: int = 1):
    try:
        task.selected_instances = []
        used_services = []
        inference_instances = task.inference_instances
        ...
        logging.error(f"Error in select_by_overall_accuracy for task {task.task_id}: {e}")
        logging.error(traceback.format_exc())
