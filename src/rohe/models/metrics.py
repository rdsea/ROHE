...
class CDMDefinition(BaseModel):
    ...
    window: str | None = Field(None, description="Evaluation window, e.g. '5m', '1h'. Required for per_period.")
...
class ClassSpecificMetric(BaseModel):
    ...
    def from_dict(cls: type[ClassSpecificMetric], data: dict[str, Any]) -> ClassSpecificMetric:
        if "performance" in data and isinstance(data["performance"], list):
            data["performance"] = [Metric(**item) if isinstance(item, dict) else item for item in data["performance"]]
        return cls.model_validate(data)
...
class RuntimePerformance(BaseModel):
    ...
    overall_performance: list[Metric] | None = Field(None, description="Overall performance metrics")
    class_specific_performance: list[ClassSpecificMetric] | None = Field(None, description="Per-class performance metrics")
    ...
    def from_dict(cls: type[RuntimePerformance], data: dict[str, Any]) -> RuntimePerformance:
        if "overall_performance" in data and isinstance(data["overall_performance"], list):
            data["overall_performance"] = [
                Metric(**item) if isinstance(item, dict) else item for item in data["overall_performance"]
            ]
