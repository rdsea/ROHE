...
class Task_Handler:
    def __init__(self, configuration, qoa_client):
        ...
        module = imp.load_source(
            self.task_conf["function"]["package"], self.task_conf["function"]["path"]
        )
        self.processing_obj = getattr(
            module, self.task_conf["function"]["module_name"]
        )(self.task_conf["config"])
        self.method_to_call = getattr(
            self.processing_obj, self.task_conf["function"]["function_name"]
        )
    ...
    def send_task_report(self):
        ...
        self.report["App-metric"]["processing_frequency"] = (
            n_request / self.heartbeat_time
        )
