...
def timed_request_executor(image_data, server_address, rate, num_requests, executor):
    timers = []
    for i in range(1, num_requests + 1):
        timer = threading.Timer(i / rate, executor.submit, [send_predict_request, image_data, server_address, i])
        timers.append(timer)
        timer.start()
...
def main(config):
    ...
    with ThreadPoolExecutor(max_workers=rate) as executor:
        timed_request_executor(image_data, server_address, rate, 10000, executor)
