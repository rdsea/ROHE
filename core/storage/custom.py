...
def execute_metric_queries(collection, metric_description, model_name, limit = 10000, timestamp = None):
    ...
    for name, query in queries.items():
        ...
        aggregation_pipeline = [{"$match": {"timestamp": {"$lt": timestamp}}}, {"$match": {"model":{"$eq": model_name}}}, {"$sort": {"timestamp": pymongo.DESCENDING}}, {"$limit": limit}]
...
def get_service_performance(collection, model_name, infrastructure):
    aggregation_pipeline = [{"$match": {"model_id": {"$eq": model_name}}}, {"$match": {"infrastructure":{"$eq": infrastructure}}}, {"$sort": {"timestamp": pymongo.DESCENDING}}, {"$limit": 1}]
    result = collection.aggregate(aggregation_pipeline)
    r_list = list(result)
    if r_list:
        return r_list[0]
...
def get_service_cost(service_cost_collection, infrastructure_cost_collection, model_name, infrastructure):
    aggregation_pipeline = [{"$match": {"model": {"$eq": model_name}}}, {"$sort": {"timestamp": pymongo.DESCENDING}}, {"$limit": 1}]
    ...
    aggregation_pipeline = [{"$match": {"infrastructure": {"$eq": infrastructure}}}, {"$sort": {"timestamp": pymongo.DESCENDING}}, {"$limit": 1}]
    infrastructure_cost_response = infrastructure_cost_collection.aggregate(aggregation_pipeline)
