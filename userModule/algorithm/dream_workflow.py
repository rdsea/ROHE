...
def compute_map_score(task: InferenceTask, remaining_time: float, starvation: float = 0.0) -> float:
    ...
    try:
        ...
        for task in task_list.data:
            if should_early_drop(task, remaining_time):
                ...
                logging.info(f"DREAM early-dropped task {task.task_id} (modality={task.modality}, min_exec={task.min_execution_time:.4f}, remaining={remaining_time:.4f})")
            else:
                task.phase = 0
                surviving_tasks.append(task)
        surviving_tasks = sorted(surviving_tasks, key=lambda t: compute_map_score(t, remaining_time), reverse=True)
