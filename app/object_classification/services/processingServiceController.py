...
class ProcessingServiceController(RoheRestObject):
    def __init__(self, **kwargs):
        ...
        self.processing_service_executor: ProcessingServiceExecutor = self.conf['processing_service_executor']
    ...
    def _handle_change_image_info_service_url(self, request):
        ...
        success = self.processing_service_executor.change_image_info_service_url(url= new_url)
        if success:
            response = f"Successfully change image info service url of the processing service"
        else:
    ...
    def _handle_change_inference_service_urls(self, request):
        ...
        success = self.processing_service_executor.change_inference_service_url(new_urls)
        if success:
            response = f"Successfully change inference service urls of the processing service"
        else:
            response = f"Something went wrong"
    ...
    def _handle_change_image_processing_function(self, request):
        ...
        success = self.processing_service_executor.change_image_processing_function(func_name= new_func_name)
        if success:
