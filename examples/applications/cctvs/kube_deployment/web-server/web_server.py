...
from core.common.restService import ImageInferenceObject, RoheRestService
...
class WebService(ImageInferenceObject):
    ...
    def imageProcessing(self, data):
        try:
            ...
            for i in range(10):
                r = rq.post(
                    url=f"http://{self.conf['url']}:{self.conf['port']}/process",
                    files={"image": image, "report": json.dumps(report)},
                )
                print(r)
                if r != None:
                    break
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Argument for Rohe Registration Service"
    )
