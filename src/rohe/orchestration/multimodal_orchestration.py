...
from userModule.algorithm.multimodal_workflow import (
...
class AdaptiveOrchestrator:
    ...
    def load_config(self):
        ...
        try:
            ...
            sla_dict = yaml.safe_load(open(self.sla_path))
            self.sla = {}
            ...
            with self.lock:
                ...
                if result:
                    for service_row in result:
                        ...
                        self.inference_service[tmp_service.inference_service_id] = (
                            tmp_service
                        )
                connection.close()
            ...
            with self.lock:
                ...
                if result:
                    for instance_row in result:
                        ...
                        self.inference_service_instance[tmp_instance.instance_id] = (
                            tmp_instance
                        )
                connection.close()
            logging.debug(
                f"{len(self.inference_service_instance)} inference service instances updated"
            )
        except Exception as e:
        ...
        try:
            ...
            for source in data_source:
                if source not in access_privileges:
                    logging.warning(
                        f"Consumer {consumer_id} does not have access to data source {source}"
                    )
            ...
            proc = multiprocessing.Process(
                ...
                args=(start_time, inference_query, tenant_sla),
            )
            proc.start()
            Timer(
                self.process_timeout, kill_process_after_timeout, args=(proc,)
            ).start()
            return {
    ...
    def execute_inference(
        ...
        tenant_sla: ServiceLevelAgreement,
    ):
        """Execute inference based on the query."""
        try:
            logging.debug(
                f"Executing inference for query on: {inference_query.data_source}"
            )
            ...
            monitoring_client = MonitoringClient(
                monitoring_url=self.db_path, metric_table=self.monitoring_table
            )
            for modality in inference_query.data_source:
                task_id = str(uuid.uuid4())
                inf_task = InferenceTask(
                    task_id=task_id,
                    modality=modality,
                    status="pending",
                    phase=0,
            ...
            self.prepare_process = multiprocessing.Process(
                target=self.prepare_data, args=(task_list,)
            )
            self.prepare_process.start()
            self.prepare_process.join()

            task_list = self.get_inf_task_info(task_list)
            e2e_response_time = inference_query.constraint.get(
                CommonMetric.RESPONSE_TIME.value, 5
            )
            inference_result = InferenceResult(inference_time=time.perf_counter())
            step_1_execution_time = time.perf_counter() - start_time
            logging.debug(
                f"######## Step 1 execution time: {step_1_execution_time:.4f} seconds"
            )
            ...
            self.task_execution_loop(
                ...
                intermediate_result=inference_result,
            )
            ...
            logging.info(
                f"Total inference time: {inference_time:.4f} seconds, Violation: {violation}"
            )
    ...
    def task_execution_loop(
        ...
        try:
            ...
            if self.check_pending_tasks(task_list) and not time_violation:
                ...
                logging.debug(
                    f"Remaining time for inference: {remaining_time:.2f} seconds"
                )
                if remaining_time <= 0:
                    time_violation = True
                    logging.warning(
                ...
                if phase_0_tasks:
                    ...
                    with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                        ...
                        remaining_time = e2e_response_time - (
                        ...
                        try:
                            ...
                            for future in as_completed(futures):
                                ...
                                if first_finished:
                                    ...
                                    recurrent_task_execution = executor.submit(
                                        self.task_execution_loop,
                                        task_list=task_list,
                                        inference_query=inference_query,
                                        start_time=start_time,
                            ...
                            step_4_execution_time = (
                                time.perf_counter() - step_4_start_time
                            )
                            logging.debug(
                        ...
                        if recurrent_task_execution != None:
                            recurrent_task_status = (
                                recurrent_task_execution.result()
                            )  # Wait for the recursive call to finish
                            if recurrent_task_status == None:
                                logging.debug(
                                    f"Number of remaining tasks: {len(task_list.data)}"
                                )
                            else:
                                logging.debug(
                                    f"Recurrent task execution status: {recurrent_task_status}"
                                )
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for task in task_list.data:
                    ...
                    futures.append(
                        executor.submit(
                            self.send_data_request, modality, time_window, task_id
                        )
                    )
                for future in as_completed(futures):
                    try:
            ...
            if not inf_id_dict:
                logging.warning(
                    "No data was prepared for inference. Check data sources and time window."
                )
            return inf_id_dict
        except Exception as e:
    ...
    def determine_execution_workflow(
        ...
        try:
            ...
            for task in task_list.data:
                logging.debug(
                    f"Modality: {task.modality}, Services: {len(task.inference_services)}, Instances: {len(task.inference_instances)}, Min Execution Time: {task.min_execution_time}, Max Execution Time: {task.max_execution_time}, Allocated Time: {task.allocated_time}, Phase: {task.phase}, Status: {task.status}"
                )
        ...
        try:
            ...
            for key_id, instance in self.inference_service_instance.items():
                if (
                    ...
                    and instance.status == InstanceStatus.AVAILABLE.value
                ):  # not in contention and failed
                    if instance.runtime_performance == []:
                        raise ValueError(
                            f"Runtime performance for instance {instance.instance_id} is empty."
                        )
            ...
            logging.error(
                f"Failed to find minimum execution time for modality {modality}: {e}"
            )
            traceback.print_exc()

    def get_time_allocation(
        self, consumer_id: str, task_list: TaskList, remaining_time: float
    ):
        try:
            ...
            for task in task_list.data:
                ...
                if task.allocated_time is None:
                    if modality in time_proportions:
                        task.allocated_time = (
                            time_proportions[modality] * remaining_time
                        )
                    else:
                    ...
                    if metric.metric_name == CommonMetric.RESPONSE_TIME.value:
                        if metric.condition == Explainability.DISABLED.value:
                            if metric.value > task.allocated_time:
                                ...
                                if service_id not in list(
                                    task.inference_services.keys()
                                ):
                                    task.inference_services[service_id] = copy.deepcopy(
                                        self.inference_service[service_id]
                                    )
                            ...
                            if metric.value > task.allocated_time:
                                ...
                                service_id = instance.inference_service_id
                                if service_id not in list(
                                    task.inference_services_ex.keys()
                                ):
                                    task.inference_services_ex[service_id] = (
                                        copy.deepcopy(
                                            self.inference_service[service_id]
                                        )
                                    )
    ...
    def distribute_inference_workload(
        self, task_list: TaskList, intermediate_result: InferenceResult
    ):
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                futures = {
                    executor.submit(
                        self.select_ensemble, task, intermediate_result
                    ): task
                    for task in task_list.data
                }
                for future in as_completed(futures):
            ...
            for task in task_list.data:
                logging.debug(
                    f"Task {task.task_id}, number of selected instances: {len(task.selected_instances)}"
                )
            return task_list
        except Exception as e:
    ...
    def select_ensemble(
        self, task: InferenceTask, intermediate_result: InferenceResult
    ):
