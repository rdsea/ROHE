...
class ConfigMap(object):
    ...
    def create(self):
        try:
            self.configmap = self.api_client.create(body=self.map_config, namespace=self.namespace)
    ...
    def get(self):
        try:
            self.configmap = self.api_client.create(name=self.name, namespace=self.namespace)
            print("ConfigMap created: %s" % self.configmap)
    ...
    def update(self, map_config):
        try:
            self.configmap = self.api_client.patch(body=map_config, name=self.name, namespace=self.namespace)
            print("Configmap created: %s" % self.configmap)
        except Exception as e:
