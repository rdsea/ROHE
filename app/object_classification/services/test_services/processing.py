...
if __name__ == "__main__":
    ...
    image_info_tags = config["external_services"]["service_registry"]["service"][
        "image_info"
    ]["tags"]
    image_info_query_type = config["external_services"]["service_registry"]["service"][
        "image_info"
    ]["type"]
    # for now, assume that we just need one image info service
    service_info = pipeline_utils.handle_service_query(
        consul_client=consul_client,
        service_name="image_info",
        query_type=image_info_query_type,
        tags=image_info_tags,
    )[0]
    ...
    # inference services
    inference_tags = config["external_services"]["service_registry"]["service"][
        "inference"
    ]["tags"]
    inference_query_type = config["external_services"]["service_registry"]["service"][
        "inference"
    ]["type"]

    inference_service_info = pipeline_utils.handle_service_query(
        consul_client=consul_client,
    ...
    processing_service = ProcessingService(config=config, port=port, endpoint=endpoint)
